...
这份 2023 说明只提供基础暴击、基础易伤和一般乘法结构，不能直接覆盖 Lord of Hatred /
Season 13 新增的命名乘区。3.1.0 当前模型还必须区分四个“同名词条先相加”的桶：

- 暴击伤害增倍；
- 易伤伤害增倍；
- 持续伤害增倍；
- 所有伤害增倍与本次事件匹配的物理/元素/武器宝石增倍。

这些桶之间再相乘；其他 `[x]` 来源只有经过词条语义或实测确认为独立威能时才单独
相乘。完整证据、分支公式、面板顶部合成值陷阱和精铸隐藏取整见
[3.1.0 / Season 13 伤害公式证据与模型](season13-damage-formula.md)。
...
官方公开的是规则片段和补丁差分，而不是完整的 S14 闭式公式。因此不能只靠这些页面
推导所有技能的最终命中值；当前闭式模型必须同时声明技能事件、条件分支、目标承伤和
实测来源。
